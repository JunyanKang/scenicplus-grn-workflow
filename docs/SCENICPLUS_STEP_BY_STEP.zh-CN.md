...
长时间 Cluster-Buster scanning 可能要等一个或多个 worker 返回后才打印
motif-level 进度。wrapper 会在官方脚本仍在运行时写出轻量 heartbeat。
间隔由 `inputs/cistarget_db_params.tsv` 中的 `heartbeat_seconds` 控制，
默认 600 秒。服务器日志希望安静时可调高，主动排错时可调低。

partial jobs 默认顺序运行。这是有意设计：官方 partial run 仍然会初始化较大的
region-by-motif score 结构，同时启动多个 partial 会复制内存压力。若机器还有
CPU 余量，更稳的方式是保持一个 active partial，让 `n_cpu=auto` 增加该
partial 内部的 `cbust` worker 数。
